...
}else{$sql=sprintf("SELECT * FROM order_fake LIMIT %s, %s",($page-1)*$perPage,$perPage);}
...
<!-- 搜尋列 -->
  ...
  <tbody>
	...
	<tr>
	<td><a href="eval_add.php?sid=<?=$r["id"]?>"><button type="button" class="btn btn-primary"> 評論</button></a></td>
	<td><a href="javascript: completeIt(<?=$r["id"]?>)"><button type="button" class="btn btn-success"> 完成</button></a></td>
		<td><?=$r["id"]?></td>
		<td><?=$r["sellername"]?></td>
      <td><?=$r["buyername"]?></td>
      <td><?=$r["total_price"]?></td>
